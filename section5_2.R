...
if(T){ 
  ...
  matplot(sig.vec, etp1, type="o", pch=1:5, lwd=2, main=expression(paste("b)" ,"ETP* Comparison varing ", sigma[max])), xlab=expression(sigma[max]), ylab="ETP*",ylim = c(1000,1800))
#  
  matplot(sig.vec, etp1.old*1000, type="o", pch=1:5, lwd=2, main=expression(paste("c)ETP Comparison varing ", sigma[max])), xlab=expression(sigma[max]), ylab="ETP",ylim = c(0.2,0.8)*1000)
